...
def test_ping(orchestrator):
    response = requests.get('{}/ping/9/8'.format(orchestrator))
    result = response.json()
    print(result)
...
def test_transfer(sender, receiver, orchestrator, num_workers = 8):   
    ...
    response = requests.post('{}/transfer/nuttcp/9/8'.format(orchestrator),json=data) 
    result = response.json()
    #print(result)
...
#def __init__(self, name, man_addr　= データを受け取るアドレス？, data_addr　= ホストマシン自体？, mon_addr = モニタリング？, interface):
# sender = DTN('?','162.244.229.51:5000', '74.114.96.105', '162.244.229.51:9100',  'enp175s0f0.555')
# receiver = DTN('dtn098', '165.124.33.174:5000', '74.114.96.98', '165.124.33.174:9100', 'vlan555')
# orchestrator = 'http://162.244.229.51:5002'
# monitor = "http://165.124.33.158:9091"

receiver = DTN('ottdtn','74.114.96.105:5000', '74.114.96.105', '66.46.187.107:9100',  'vlan555')
sender = DTN('dtn098', '74.114.96.98:5000', '74.114.96.98', '165.124.33.174:9100', 'vlan555')
orchestrator = 'http://74.114.96.98:7000'
monitor = "http://165.124.33.158:9091"
