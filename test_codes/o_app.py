...
SQLALCHEMY_DATABASE_URI = "sqlite:///../orchestrator/db/dtn.db"
...
def transfer_job(sender, sender_data_ip, receiver, srcfile, dstfile, tool, snd_data, rcv_data, timeout=None, retry = 5):
    for i in range(0, retry):
        result = run_transfer(sender, sender_data_ip, receiver, srcfile, dstfile, tool, snd_data, rcv_data)
...
def run_transfer(sender_ip, sender_data_ip, receiver_ip, srcfile, dstfile, tool, snd_params, rcv_params):
    global gparams

    try:        
        # logging.debug('Running sender')
        ## sender
        snd_params['file'] = srcfile
        snd_params['blocksize'] = gparams['blocksize']
        if srcfile == None and 'duration' not in snd_params:
            abort(make_response(jsonify(message="Need duration for mem-to-mem transfer"), 400))
        response = requests.post('http://{}/sender/{}'.format(sender_ip, tool), json=snd_params)
        ...
        if srcfile == None:
            result['duration'] = snd_params['duration']
        else:
...
def transfer(tool,sender_id, receiver_id):
    ...
    if 'blocksize' in data:
        try: 
            ...
            gparams['blocksize'] = data['blocksize']
        except Exception:
    ...
    try:
        ...
        traceback.print_exc()
        abort(make_response(jsonify(message="Error occured during DB commit"), 400))

    # start_time = datetime.datetime.utcnow()
    executor = libs.ThreadExecutor.ThreadPoolExecutor(max_workers=num_workers)
    
    future_to_transfer = {
        executor.submit(transfer_job, sender.man_addr, sender.data_addr, receiver.man_addr, srcfile, dstfile, tool, copy.deepcopy(data), copy.deepcopy(rcv_data), timeout): 
